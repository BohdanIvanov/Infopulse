...
// first option
   ...
   c.increase = function(){
    ...
    c.put = function(jqGood, bDelete){
		...
		if (bDelete){		// if delete
			...
			if(!jqCartGood.length){         // no goods in cart
				...
				//to write                              //to read
				jqCartGood.find('.b-good__name').html( jqGood.find('.b-good__name').html() ); // name
				jqCartGood.find('.b-good__price').html( jqGood.find('.b-good__price').html()); // price
				jqCartGood.find('.b-good__image').attr('src', jqGood.find('.b-good__image').attr('src')); // photo
				jqCartGood.addClass(sGoodIdClassName);
			}
			
			// the product is already in cart
			jqCartGood.find('.b-order-form__quantity').val( jqGood.find('.b-order-form__quantity').val());//quantity
    ...
    // Second option
	// to cart object (c.elem) Adding minicart object, searching buttons "+" и "-"....
